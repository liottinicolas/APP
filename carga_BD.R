...
# 
# 
# ver_asd <- historico_llenado %>% 
#   inner_join(unique_gids_df , by="gid") %>% 
#   filter(Fecha >= "2025-02-09") %>% 
#   distinct(gid,Direccion,the_geom)
# 
# ubicaciones_diferente_direccion_y_calle_final <- ubicaciones_diferente_direccion_y_calle %>% 
#   left_join(ver_asd %>% select(gid, Direccion, the_geom) %>% distinct(),
#             by="gid")
...
# gids_no_existentes <- ubicaciones_diferente_direccion_y_calle %>%
#   anti_join(historico_llenado, by = "gid") %>%
#   distinct(gid)
# 
# # elimino los gids que ya no están más vigentes al df.
# ubicaciones_diferente_direccion_y_calle <- ubicaciones_diferente_direccion_y_calle %>%
#   anti_join(gids_no_existentes, by = "gid")

###### Busco aquellas ubicaciones que para un mismo GID no cambiaron nunca.
# 
# ## Aca calcula la cantidad de cambios de direccion que tuvo por cada gid.
# ubicaciones_que_no_cambiaronnunca_de_direccion <- historico_llenado %>%
#   semi_join(ubicaciones_diferente_direccion_y_calle, by = "gid") %>%  # Filtra por los gid deseados
#   arrange(gid, Fecha) %>%                                             # Ordena por gid y fecha
#   group_by(gid) %>%
#   summarise(cambios_direccion = sum(Direccion != lag(Direccion), na.rm = TRUE)) %>%  
#   ungroup()
# 
# ## Obtengo the_Geom por las gid filtradas
# geom_por_gid <- historico_llenado %>%
#   group_by(gid) %>%
#   summarise(the_geom_historico = first(the_geom), .groups = "drop")
# 
# 
# ubicaciones_que_no_cambiaronnunca_de_direccion <- ubicaciones_diferente_direccion_y_calle %>%
#   left_join(ubicaciones_que_no_cambiaronnunca_de_direccion, by = "gid") %>%         # Une la cantidad de cambios de dirección
#   left_join(geom_por_gid, by = "gid") %>%         # Une el the_geom de historico_llenado
#   mutate(
#     the_geom = if_else(cambios_direccion == 0, the_geom_historico, the_geom)
#   ) %>%
#   select(-cambios_direccion, -the_geom_historico)  # Elimina columnas auxiliares
# 
# NA_ubicaciones_que_no_cambiaronnunca_de_direccion <- ubicaciones_que_no_cambiaronnunca_de_direccion %>% 
#   filter(is.na(the_geom))
# 
# bloque_3_misma_direccion_siempre <- ubicaciones_que_no_cambiaronnunca_de_direccion %>% 
#   anti_join(NA_ubicaciones_que_no_cambiaronnunca_de_direccion, by="gid")
# 
# #######################################
# 
# ubicaciones_complicadas <- NA_ubicaciones_que_no_cambiaronnunca_de_direccion
# 
