...
incidenciasMantenimientoUI <- function(id) {
  ...
  tagList(
    ...
    fluidRow(
      ...
      downloadButton(ns("exportar_excel_mantenimiento"), "Exportar a Excel", 
...
incidenciasMantenimientoServer <- function(input, output, session) {
  ...
  filtrado_reactivo_Mantenimiento <- reactive({
    ...
    print(input$checkbox_Mantenimiento)
    ...
    if (!is.null(input$checkbox_Mantenimiento) && length(input$checkbox_Mantenimiento) > 0) {
      df_final <- df_final %>%
        filter(Incidencia %in% input$checkbox_Mantenimiento)
  ...
  output$exportar_excel_mantenimiento <- downloadHandler(
    filename = function() {
      paste("Incidencias_mantenimiento_hasta", "_", ultima_fecha_registro, ".xlsx", sep = "")
    },
